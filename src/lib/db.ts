// ── Generic .venturewiki file listing and fetching ─────────────────────────────

/**
 * Recursively lists all files and folders in the .venturewiki directory of a repo.
 * Returns a tree structure: { name, path, type: 'file'|'dir', children? }
 */
export async function getVenturewikiFiles(slug: string, dir = '.venturewiki'): Promise<any[]> {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      return await Promise.all(data.map(async (item: any) => {
        if (item.type === 'dir') {
          return {
            ...
            path: item.path,
            type: 'dir',
            children: await getVenturewikiFiles(slug, item.path),
          }
        } else {
          return {
            name: item.name,
      ...
      // Single file (should not happen for .venturewiki root)
      return []
    }
  } catch {
    return []
...
/**
 * Fetches the content and metadata of a file in .venturewiki (any type).
 * Returns { name, path, type, content, encoding, size, download_url }
 */
export async function getVenturewikiFileContent(slug: string, filePath: string): Promise<any | null> {
  const octokit = getPublicOctokit()
  try {
    const { data } = await octokit.rest.repos.getContent({
      owner: GITHUB_ORG,
      repo: slug,
      path: `.venturewiki/${filePath.replace(/^\/?/, '')}`,
    })
