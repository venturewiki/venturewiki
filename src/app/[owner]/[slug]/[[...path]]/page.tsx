...
export default function VenturePage() {
  ...
  // globally unique (same as GitHub's /:owner/:repo pattern).
  ...
  // A multi-segment path (e.g. /owner/slug/docs/readme.md) is a file inside a subfolder.
  ...
  // Navigation helpers use the /{owner}/{slug} URL pattern (GitHub-style).
  const selectTab = (id: string) => {
    const target = id === defaultTab ? `/${owner}/${slug}` : `/${owner}/${slug}/${id}`
    ...
    <div className="min-h-screen bg-ink">
      ...
      <div className="border-b border-rule">
        <div className="max-w-7xl mx-auto px-4 py-2.5 flex items-center gap-1.5 text-xs text-muted">
          ...
          <Link href={`/${owner}`} className="hover:text-paper transition-colors font-mono">@{owner}</Link>
          <ChevronRight className="w-3 h-3" />
